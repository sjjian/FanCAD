...
void main() {
  ...
  test('splines, leaders, images and array inserts survive DXF', () {
    final original = drawing(
      blocks: const [BlockRecord(name: 'CELL', entityIds: [])],
      entities: [
        SplineEntity(
          id: 0,
  ...
  test('paper layouts and viewports survive DXF', () {
    final original = drawing(
      entities: const [
        LineEntity(id: 0, start: Vec2.zero(), end: Vec2(80, 0)),
      ],
    )
      ..addLayout(
        const Layout(
  ...
  test('dimension styles survive DXF', () {
    ...
    final style = const DxfReader().readString(dxf).namedDimStyle('ARCH')!;
  ...
  test('a hatch keeps the definition lines the file resolved for it', () {
    ...
    final lines = rt.dxf(original).entities.whereType<HatchEntity>().single
        .patternLines;
  ...
  test('a dimension keeps its style and definition points through DXF', () {
    final original = drawing(
      dimStyles: const [DimStyleDef(name: 'ARCH', decimalPlaces: 0)],
  ...
  test('a non-plottable layer stays off the plot through DXF', () {
    final original = drawing(
      layers: const [LayerDef(name: 'VIEWPORT-FRAME', plottable: false)],
    );
  ...
  test('text styles survive DXF', () {
    ...
    final restored = rt.dxf(original);
  ...
  test('entity and layer line weights survive DXF', () {
    final original = drawing(
      layers: const [LayerDef(name: 'THICK', lineWeight: 50)],
      entities: const [
        LineEntity(
          id: 0,
          start: Vec2.zero(),
  ...
  test('a hidden entity stays off through DXF', () {
    ...
    final dxf = const DxfWriter().writeString(original);
    expect(dxf, contains('\n60\n1\n'));
    expect(const DxfReader().readString(dxf).entities.single.props.visible, isFalse);
  });
  ...
  test('a hidden layer stays off through DXF', () {
    ...
    final dxf = const DxfWriter().writeString(original);
    expect(dxf, contains('\n-3\n'));
    final layer = const DxfReader().readString(dxf).layers['CONSTRUCTION'];
  ...
  test('line type dash patterns survive DXF', () {
    final original = drawing(
      ...
      layers: const [LayerDef(name: 'AXIS', lineType: 'CENTER')],
      entities: const [
        LineEntity(
          id: 0,
          start: Vec2.zero(),
