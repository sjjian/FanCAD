...
void main() {
  ...
  test('a leftover long first line becomes a short session title', () {
    ...
    expect(
      titleFromUserMessage('${'x' * 80}\nsecond line'),
      '${'x' * 39}…',
    );
    expect(titleFromUserMessage('${'x' * 80}'), isNot(contains('second')));
  });

  testWidgets('new-session leftover sits after the last tab, not the strip end', (
    tester,
  ) async {
    final ai = controller();
    ...
    final plus = tester.getRect(find.byKey(const Key('assistant-new-session')));
  ...
  testWidgets('closing a leftover tab keeps the other thread', (tester) async {
    ...
    await tester.tap(find.byKey(Key('assistant-session-close-${ai.activeChat.id}')));
    await tester.pump();
