...
class FcbReader {
  ...
  Map<String, LayerDef> _decodeLayers(List<LineTypeDef> lineTypes) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      final name = _string(_view.getUint32(at + FcbLayer.name, Endian.little));
      ...
      result[name] = LayerDef(
        ...
        transparency: _view.getInt32(at + FcbLayer.transparency, Endian.little),
      );
    }
    return result;
  }
  ...
  void _applyLayouts(CadDocument document, List<String> blockNames) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      document.addLayout(
        Layout(
          name: _string(_view.getUint32(at + FcbLayout.name, Endian.little)),
          ...
          plotRotation: ((flags >> FcbLayoutFlags.plotRotationShift) & 3) * 90,
  ...
  CadEntity? _decodeEntity(
    ...
    final geomOffset = _view.getUint64(
      at + FcbEntity.geomOffset,
      Endian.little,
    );
    ...
    String stringAt(int index) =>
        index < stringCount ? _string(stringOffset + index) : '';

    final layerIndex = _view.getUint32(
      at + FcbEntity.layerIndex,
      Endian.little,
    );
    final lineTypeIndex = _view.getUint32(
    ...
    switch (type) {
      ...
      case FcbType.dimension:
        ...
        return DimensionEntity(
          ...
          sourceIds: [for (var i = 2; i < ints.length; i++) ints[i]],
        );
