...
class _StringTable {
  ...
  int intern(String value) => _indexOf.putIfAbsent(value, () {
    _values.add(value);
  ...
  Uint8List encode() {
    final encoded = [for (final value in _values) utf8.encode(value)];
    var dataLength = 0;
...
class FcbWriter {
  ...
  Uint8List _encodeEntity(
    ...
    view.setFloat64(
      FcbEntity.minX,
      bounds.isEmpty ? 0 : bounds.minX,
      Endian.little,
    );
    view.setFloat64(
      FcbEntity.minY,
      bounds.isEmpty ? 0 : bounds.minY,
      Endian.little,
    );
    view.setFloat64(
      FcbEntity.maxX,
      bounds.isEmpty ? 0 : bounds.maxX,
      Endian.little,
    );
    view.setFloat64(
      FcbEntity.maxY,
      bounds.isEmpty ? 0 : bounds.maxY,
      Endian.little,
    );
    ...
    switch (entity) {
      ...
      case TextEntity():
        ...
        return _Payload(
          ...
          geomCount: 6,
          intOffset: _ints.addAll([entity.hAlign.index, entity.vAlign.index]),
          intCount: 2,
      ...
      case HatchEntity():
        ...
        final coordinates = <double>[entity.patternAngle, entity.patternScale];
  ...
  _Payload _geom(List<double> values) =>
      _Payload(geomOffset: _doubles.addAll(values), geomCount: values.length);
  ...
  Uint8List _encodeLineTypes(CadDocument document, List<String> names) {
    final buffer = Uint8List(alignUp8(8 + names.length * FcbRecord.lineType));
    final view = ByteData.view(buffer.buffer);
    view.setUint64(0, names.length, Endian.little);
    for (var i = 0; i < names.length; i++) {
  ...
  Uint8List _encodeBlocks(
    ...
    for (var i = 0; i < names.length; i++) {
      final name = names[i];
      final block =
          document.blocks[name] ?? BlockRecord(name: name, isLayoutBlock: true);
  ...
  Uint8List _encodeLayouts(CadDocument document, Map<String, int> blockIndex) {
  ...
  Uint8List _encodeHeaderVariables(CadDocument document) {
    ...
    view.setUint64(0, entries.length, Endian.little);
    for (var i = 0; i < entries.length; i++) {
      view.setUint32(8 + i * 8, _strings.intern(entries[i].key), Endian.little);
      view.setUint32(
        8 + i * 8 + 4,
  ...
  static int _packEntityColor(CadColor color) => packColor(switch (color.kind) {
