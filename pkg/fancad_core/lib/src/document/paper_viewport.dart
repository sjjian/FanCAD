...
abstract class PaperViewport with _$PaperViewport {
  ...
  const factory PaperViewport({
    ...
    @JsonKey(
      name: 'paper',
      fromJson: _paperBoundsFromJson,
      toJson: _paperBoundsToJson,
    )
    ...
    @JsonKey(
      name: 'center',
      fromJson: _modelCenterFromJson,
      toJson: _modelCenterToJson,
    )
    required Vec2 modelCenter,

    /// Model units per paper unit. 1 = 1:1, 0.1 = 1:10.
    @Default(1) double scale,
    @JsonKey(toJson: _omitZero) @Default(0) double rotation,
    @JsonKey(name: 'on', toJson: _omitTrue) @Default(true) bool isOn,
    @JsonKey(toJson: _omitFalse) @Default(false) bool locked,
    @JsonKey(toJson: _omitLayerZero) @Default('0') String layer,
  ...
  PaperViewport withGrip(int index, Vec2 paper) {
    ...
    final next = switch (index) {
      ...
      1 => Bounds2.fromCorners(
        Vec2(box.minX, paper.y),
        Vec2(paper.x, box.maxY),
      ),
      2 => Bounds2.fromCorners(Vec2(box.minX, box.minY), paper),
      3 => Bounds2.fromCorners(
        Vec2(paper.x, box.minY),
        Vec2(box.maxX, paper.y),
      ),
