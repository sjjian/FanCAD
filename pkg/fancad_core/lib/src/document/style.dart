...
abstract class EntityProps with _$EntityProps {
  ...
  const factory EntityProps({
    ...
    @JsonKey(toJson: _omitByLayerName) @Default('ByLayer') String lineType,
    @JsonKey(toJson: _omitByLayerWeight)
    @Default(LineWeight.byLayer)
    int lineWeight,
    @JsonKey(toJson: _omitOne) @Default(1) double lineTypeScale,

    /// -1 means inherit from the layer.
    @JsonKey(toJson: _omitMinusOne) @Default(-1) int transparency,
    @JsonKey(toJson: _omitTrue) @Default(true) bool visible,

    /// Z offset preserved for DWG round-tripping in this 2D application.
    @JsonKey(toJson: _omitZero) @Default(0) double elevation,
  }) = _EntityProps;
...
Object? _colorToJson(CadColor color) => switch (color.kind) {
  ...
  ColorKind.trueColor => '#${color.value.toRadixString(16).padLeft(6, '0')}',
};
