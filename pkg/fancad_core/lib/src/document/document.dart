...
class CadDocument implements BlockLookup, StyleResolver {
  CadDocument({
    ...
    _lineTypes.addAll(lineTypes ?? {'Continuous': LineTypeDef.continuous});
    _textStyles.addAll(textStyles ?? {'Standard': TextStyleDef.standard});
    _dimStyles.addAll(dimStyles ?? {'Standard': DimStyleDef.standard});
    _blocks.putIfAbsent(
      this.modelSpaceBlockName,
      () => BlockRecord(name: this.modelSpaceBlockName, isLayoutBlock: true),
    );
    if (_layouts.isEmpty) {
      _layouts.add(
        Layout(
  ...
  InsUnits get insUnits => InsUnits.fromHeader(_headerVariables[r'$INSUNITS']);
  ...
  Map<String, String> get headerVariables => Map.unmodifiable(_headerVariables);
  ...
  CadEntity insertEntity(
    ...
    if (block == null) {
      _blocks[blockName] = BlockRecord(name: blockName, entityIds: [entity.id]);
    } else {
  ...
  bool removeLayout(String name) {
    ...
    if (_activeLayoutName == name) {
      _activeLayoutName = _layouts.firstWhere((item) => item.isModelSpace).name;
    }
    ...
    if (!blockStillUsed) {
      ...
      if (block != null && block.isLayoutBlock && block.entityIds.isEmpty) {
  ...
  List<int> queryVisible(Bounds2 box) =>
      indexFor(currentBlockName).search(box).where((id) {
        final entity = _entities[id];
        return entity != null &&
  ...
  List<CadEntity> entitiesOf(String blockName) {
    ...
    return [for (final id in block.entityIds) ?_entities[id]];
  }
  ...
  Bounds2 boundsOfEntity(CadEntity entity) =>
  ...
  Bounds2 indexBoundsOf(CadEntity entity) =>
      entity.indexBounds(blocks: this, tolerance: defaultTolerance);
  ...
  Bounds2 get extents {
    ...
    final sheet = Bounds2(
      0,
      0,
      activeLayout.paperWidth,
      activeLayout.paperHeight,
    );
  ...
  ResolvedStyle resolve(EntityProps props, ResolvedStyle inherited) {
    ...
    final color = switch (props.color.kind) {
      ColorKind.byLayer => layerDef?.color ?? const CadColor.indexed(7),
