...
class Transaction {
  ...
  bool removeLayer(String name) {
    ...
    final inUse = document.entities.any((entity) => entity.props.layer == name);
  ...
  void putLineType(LineTypeDef lineType) =>
      _run(PutLineTypePatch(lineType, document.lineTypes[lineType.name]));
  ...
  void setHeaderVariable(String key, String value) =>
      _run(HeaderVariablePatch(key, value, document.headerVariables[key]));

  void setActiveLayout(String name) =>
      _run(ActiveLayoutPatch(name, document.activeLayoutName));

  /// Creates or replaces a layout tab. Undo of a newly created tab
