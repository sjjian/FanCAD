...
void main() {
  ...
  test(
    'a second paste that imports a new layer still draws the first paste',
    () {
      ...
      final firstIds = clipWithLayer(
        'WALL',
        Vec2.zero(),
      ).paste(first, insertion: Vec2.zero());
      first.commit();
      expect(target.queryVisible(const Bounds2(-1, -1, 5, 1)), firstIds);

      final second = Transaction(target, label: 'Paste');
      final secondIds = clipWithLayer(
        'ROOF',
        const Vec2(10, 0),
      ).paste(second, insertion: const Vec2(10, 0));
      second.commit();
