...
void main() {
  ...
  test('a bulk load of empty boxes cannot invent window hits', () {
    final index = SpatialIndex()
      ..bulkLoad({1: const Bounds2.empty(), 2: const Bounds2.empty()});
  ...
  test('finds only the entities inside the query box', () {
    ...
    final entries = <int, Bounds2>{
      for (var i = 0; i < 500; i++) i: Bounds2(i * 10, 0, i * 10 + 5, 5),
    };
  ...
  test('window search requires full containment', () {
    final index = SpatialIndex()
      ..bulkLoad({1: const Bounds2(0, 0, 2, 2), 2: const Bounds2(1, 1, 9, 9)});
    expect(index.searchContained(const Bounds2(0, 0, 4, 4)), [1]);
  });

  test('point search expands the query by the snap tolerance', () {
    final index = SpatialIndex()..bulkLoad({1: const Bounds2(10, 10, 11, 11)});
