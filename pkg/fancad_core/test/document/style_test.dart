...
void main() {
  group('CadColor', () {
    test('JSON round-trips every kind and the public helpers', () {
      ...
      expect(
        {const CadColor.indexed(1)}.contains(const CadColor.indexed(1)),
        isTrue,
      );
    });
  });
  ...
  group('TextStyleDef', () {
    test('SHX families are the ones that need a stroke renderer', () {
      ...
      expect(
        const TextStyleDef(name: 'A', fontFamily: 'arial.shx').isShxFont,
        isTrue,
      );
      expect(
        const TextStyleDef(name: 'A', fontFamily: 'Arial').isShxFont,
        isFalse,
      );
      expect(
        const TextStyleDef(name: 'A', fontFamily: 'txt').isShxFont,
        isTrue,
      );
      expect(
        const TextStyleDef(name: 'A', fontFamily: 'ROMANS').isShxFont,
        isTrue,
      );
  ...
  group('DimStyleDef', () {
    test('scale and decimal clamps reject broken values', () {
      ...
      expect(
        const DimStyleDef(name: 'X', decimalPlaces: -1).clampedDecimals,
        0,
      );
      expect(
        const DimStyleDef(name: 'X', decimalPlaces: -3).clampedDecimals,
        0,
      );
      expect(
        const DimStyleDef(name: 'X', decimalPlaces: 99).clampedDecimals,
        8,
      );
    ...
    test(
      'header DIMSCALE fills an identity style so fallback text is readable',
      () {
        final document = CadDocument()..setHeaderVariable(r'$DIMSCALE', '14');
        expect(
          document.dimStyle('Standard').scaledTextHeight,
          closeTo(35, 1e-9),
        );
    ...
    test('regenerated dimensions follow the named DIMSTYLE', () {
      ...
      expect(
        tip.distanceTo(left),
        closeTo(8 * math.sqrt(1 + 0.35 * 0.35), 1e-9),
      );
    });
    ...
    test('explode uses the style for text height and decimals', () {
      ...
      final pieces = Construct.explodeDimension(
        dim,
        style: const DimStyleDef(name: 'ARCH', textHeight: 5, decimalPlaces: 0),
      );
