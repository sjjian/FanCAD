...
void main() {
  ...
  group('EmitContext', () {
    test('apply, descend and local tolerance follow the block transform', () {
      ...
      expect(
        moved.localTolerance(const Mat3.scaling(2, 2)),
        closeTo(0.05, 1e-12),
      );
      ...
      expect(
        const EmitContext(
          tolerance: 0.1,
          minExtent: 4,
        ).descend(const Mat3.scaling(2, 2), ResolvedStyle.fallback).minExtent,
        4,
      );
      expect(
        const EmitContext(
          tolerance: 0.1,
          minExtent: 4,
        ).isSubPixelWorld(const Bounds2(0, 0, 3, 3)),
        isTrue,
      );
      expect(
        const EmitContext(
          tolerance: 0.1,
          minExtent: 4,
        ).isSubPixelWorld(const Bounds2(0, 0, 10, 1)),
        isFalse,
      );
  ...
  group('sinks', () {
    ...
    test(
      'PolylineSink skips empty runs and records holes and image frames',
      () {
        ...
        sink.fill(
          ...
          style,
          holes: [
            Float64List.fromList([2, 2, 4, 2, 4, 4, 2, 4]),
          ],
        );
