...
void main() {
  ...
  group('CadDocument', () {
    ...
    test(
      'queryVisible still finds earlier entities after a new layer is added',
      () {
        final document = CadDocument();
        final first = document.addEntity(
          const LineEntity(id: 0, start: Vec2.zero(), end: Vec2(4, 0)),
    ...
    test(
      'a world-coord insert cannot stretch Zoom Extents among local lines',
      () {
        final document = CadDocument()..putBlock(const BlockRecord(name: 'bk'));
        for (var i = 0; i < 20; i++) {
          document.addEntity(
            LineEntity(id: i, start: Vec2(i * 10, 0), end: Vec2(i * 10 + 5, 1)),
          );
        }
        document
          ..addEntity(
    ...
    test(
      'an insert frames local geometry after subtracting the block base',
      () {
        final document = CadDocument()
