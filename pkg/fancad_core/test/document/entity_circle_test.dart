...
void main() {
  ...
  test(
    'circle quadrant grips change radius and a non-uniform scale becomes an ellipse',
    () {
      const circle = CircleEntity(id: 1, center: Vec2.zero(), radius: 5);
      ...
      expect(circle.withGrip(0, const Vec2(1, 1)).center, const Vec2(1, 1));
      final uniform =
          circle.transformed(const Mat3.scaling(2, 2)) as CircleEntity;
      expect(uniform.radius, 10);
      expect(uniform.center, const Vec2.zero());
      final tall =
          circle.transformed(const Mat3.scaling(1, 2)) as EllipseEntity;
      expect(tall.ratio, lessThanOrEqualTo(1));
      expect(tall.majorLength, closeTo(10, 1e-9));
      final wide =
          circle.transformed(const Mat3.scaling(2, 1)) as EllipseEntity;
