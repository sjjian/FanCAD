...
void main() {
  ...
  test('anonymous dimension blocks with distinct names all survive FCB', () {
    final restored = rt.fcb(
      drawing(
        ...
        owned: const {
          '*D\$aa': [LineEntity(id: 1, start: Vec2.zero(), end: Vec2(10, 0))],
          '*D\$bb': [LineEntity(id: 2, start: Vec2(20, 0), end: Vec2(30, 0))],
        },
      ),
    );
  ...
  test('a hatch keeps the definition lines the file resolved for it', () {
    ...
    expect(hatch.patternLines[1].dashes, [closeTo(2, 1e-9), closeTo(-1, 1e-9)]);
  });
