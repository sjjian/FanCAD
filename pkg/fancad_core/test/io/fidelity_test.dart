...
void main() {
  ...
  test('a lost xref path is not a clean round trip', () {
    ...
    final target = drawing(blocks: const [BlockRecord(name: 'BRACKET')]);
  ...
  test('a lost layer, extra sheet and extra xref are not a clean trip', () {
    final source = drawing(
      ...
      blocks: const [BlockRecord(name: 'ONLY_HERE', xrefPath: '/tmp/only.dxf')],
    );
    ...
    final target = CadDocument()
      ...
      ..putBlock(const BlockRecord(name: 'NEW_XREF', xrefPath: '/tmp/new.dxf'))
