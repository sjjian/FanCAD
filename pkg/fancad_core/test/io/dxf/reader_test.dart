...
EOF
''');
      expect(document.entities.whereType<SolidEntity>().single.corners, const [
        Vec2.zero(),
        Vec2(2, 0),
        Vec2(2, 1),
        Vec2(0, 1),
...
''');
    ...
    test(
      'a two-vertex hatch is dropped rather than stored as an empty loop',
      () {
        final document = parse('''
  0
SECTION
...
''',
    ...
    test(
      'the paper-space main viewport and a zero-size window are dropped',
      () {
        final document = parse('''
  0
SECTION
