...
void main() {
  ...
  test('a viewport plot omits layers frozen in that window', () {
    ...
    final shown = const Plotter().toSvg(
      document,
    ...
    final hidden = const Plotter().toSvg(
      document,
      layout: document.layouts.last,
    );
  ...
  test(
    'model-space SVG paints strokes, fills, text, points and image frames',
    () {
  ...
  test('a live viewport clips model ink and frames the paper window', () {
    ...
    final pdf = String.fromCharCodes(
      const Plotter().toPdf(document, layout: layout),
    );
  ...
  test('a paper viewport plot clips model geometry in SVG', () {
    ...
    expect(
      svg,
      contains('<rect x="20.0" y="-160.0" width="180.0" height="140.0"/>'),
    );
