...
/// as a static PIC library and linked into `libfancad_core`. The application
...
void main(List<String> arguments) async {
  await build(arguments, (input, output) async {
    ...
    final logger = Logger('fancad_core')
      ..level = Level.ALL
      ..onRecord.listen((record) {
        if (!verbose && record.level < Level.WARNING) return;
        stderr.writeln('[fancad_core] ${record.level.name}: ${record.message}');
      });

    final nativeDirectory = input.packageRoot.resolve('native/fancad_core/');
    final sources = [
...
Future<void> _compile({
  ...
  final builder = CBuilder.library(
    name: 'fancad_core',
    assetName: 'src/io/native/bindings.dart',
    sources: sources,
    includes: includes,
    defines: defines,
    libraries: libraries,
