...
void main() {
  ...
  eachCase(SnapMode.values, (mode) {
    expect(mode.label, isNotEmpty);
    expect(SnapMode.parse(mode.name), mode);
  }, name: (mode) => '${mode.name} has a label and parse is exact');

  test(
    'an unknown snap name stays unset and the defaults stay the everyday set',
    () {
      expect(SnapMode.parse('nope'), isNull);
      expect(
        SnapMode.defaults,
        containsAll([SnapMode.endpoint, SnapMode.midpoint]),
      );
    },
  );
  ...
  test(
    'ortho tracking projects the cursor and intersection wins on a crossing',
    () {
      final empty = CadDocument();
      ...
      final crossed = CadDocument()
        ...
        ..addEntity(
          const LineEntity(id: 0, start: Vec2(5, -5), end: Vec2(5, 5)),
        );
      final hits = SnapEngine(
        modes: {SnapMode.intersection},
      ).resolve(crossed, view, const Vec2(5.2, 0.2));
