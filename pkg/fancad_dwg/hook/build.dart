...
/// as a static PIC library and linked into `libfancad_dwg`. The application
...
void main(List<String> arguments) async {
  await build(arguments, (input, output) async {
    ...
    final logger = Logger('fancad_dwg')
      ..level = Level.ALL
      ..onRecord.listen((record) {
        if (!verbose && record.level < Level.WARNING) return;
        stderr.writeln('[fancad_dwg] ${record.level.name}: ${record.message}');
      });

    final nativeDirectory = input.packageRoot.resolve('native/fancad_dwg/');
    final sources = [
      nativeDirectory.resolve('fcb_builder.c').toFilePath(),
      nativeDirectory.resolve('fancad_dwg.c').toFilePath(),
