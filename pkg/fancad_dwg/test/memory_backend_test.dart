...
void main() {
  test('the memory backend stores FCB and throws on a missing path', () async {
    ...
    expect(backend.capabilities.readableExtensions, containsAll(['dwg', 'dxf']));

    final read = await backend.readToFcb('/mem/a.dwg');
    expect(read, bytes);

    await backend.writeFromFcb('/mem/b.dwg', Uint8List.fromList([1, 2, 3]));
    expect(await backend.readToFcb('/mem/b.dwg'), Uint8List.fromList([1, 2, 3]));
  ...
  test('the importer opens a memory DWG and can reopen it from the FCB cache', () async {
