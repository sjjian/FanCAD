...
void main() {
  ...
  test('streamed leftovers paint tokens as they arrive', () async {
    ...
    expect(
      conversation.visible.where((item) => item.role == ChatRole.assistant),
      hasLength(1),
    );
    expect(
      conversation.visible
          .singleWhere((item) => item.role == ChatRole.assistant)
          .text,
      'Hello',
    );
  });
  ...
  test('a leftover incomplete tool call retries without streaming', () async {
    final lineRegistry = CommandRegistry()
      ..register(
        CommandDescriptor(
          id: 'draw.line',
          title: 'Line',
    ...
    final lineSession = DocumentSession(id: 't', document: CadDocument());
    final provider = _IncompleteToolProvider();
    final agent = AgentLoop(
      provider: provider,
      registry: lineRegistry,
      execute: (id, args, {tab}) => lineRegistry.run(
        id,
        args: args,
        source: ChangeSource.ai,
        contextBuilder: (descriptor) => CommandContext(
          ...
          input: ArgsCommandInput(
            args: CommandArgs(args),
            params: descriptor.params,
            selection: lineSession.selection,
          ),
          source: ChangeSource.ai,
          commandId: id,
        ),
