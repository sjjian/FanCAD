...
void main() {
  test(
    'user and tool turns appear in both the panel and the LLM transcript',
    () {
  ...
  test('clear empties both sides of the transcript', () {
    ...
    expect(
      conversation.visible.singleWhere((item) => item.isError).text,
      'failed',
    );
    conversation.clear();
    expect(conversation.visible, isEmpty);
  ...
  test('leftover think tags become a reasoning card, not model history', () {
    final conversation = Conversation();
    conversation.addAssistantLlm(
      const LlmMessage.assistant('<think>use a polyline</think>Drew the tail.'),
    );
    expect(conversation.visible.map((item) => item.role), [
      ChatRole.reasoning,
      ChatRole.assistant,
  ...
  test('compactLlm rewrites the model transcript and leaves the panel', () {
    final conversation = Conversation();
    ...
    conversation.addToolResult(
      ...
      content: '{"entities":[1,2,3]}',
    );
    conversation.addUser('now the right one');
    final visibleCount = conversation.visible.length;
    conversation.compactLlm(stubOldToolResults(conversation.llmMessages));
    expect(conversation.visible, hasLength(visibleCount));
