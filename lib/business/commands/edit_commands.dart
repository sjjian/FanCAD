...
class EditCommands {
  ...
  static CommandDescriptor _scale() => CommandDescriptor(
    ...
    handler: (context) async {
      ...
      if (factor == null) {
        ...
        _installTransformPreview(context, ids, base, (cursor) {
    ...
    handler: (context) async {
      ...
      if (source2 != null && dest2 == null) {
        _installTransformPreview(
          ...
          dest1,
          (cursor) =>
              Mat3.align(source1, dest1, source2: source2, dest2: cursor),
          extra: (cursor) => [
  ...
  static CommandDescriptor _polarArray() => CommandDescriptor(
    ...
    handler: (context) async {
      ...
      return CommandResult(
        status: CommandStatus.ok,
        message:
            'Polar array: ${committed.change.added.length} copies created.',
        data: {'ids': committed.change.added},
        transaction: committed,
      );
    },
  ...
  static CommandDescriptor _renameBlock() => CommandDescriptor(
    ...
    handler: (context) async {
      ...
      if (requested.isEmpty) {
        return const CommandResult.failed(
          'RENAME needs the current block name.',
        );
      }
      final block = _insertableBlock(context.document, requested);
      if (block == null) {
  ...
  static CommandDescriptor _hatchEdit() => CommandDescriptor(
    id: 'edit.hatch',
    title: 'Hatch Edit',
    category: _category,
    aliases: const ['hatchedit', 'he'],
    description:
  ...
  static CommandDescriptor _dimensionText() => CommandDescriptor(
    ...
    handler: (context) async {
      ...
      return CommandResult(
        status: CommandStatus.ok,
        message:
            'Set text on ${committed.change.modified.length} dimension(s).',
        transaction: committed,
      );
    },
  );

  static CommandDescriptor _dimTedit() => CommandDescriptor(
    id: 'edit.dimTedit',
    title: 'Move Dimension Text',
    category: _category,
    aliases: const ['dimtedit', 'tedit'],
    description:
  ...
  static DimensionEntity _dimensionTextAt(DimensionEntity dim, Vec2 at) {
      ...
      final committed = context.edit('Edit Text', (transaction) {
        for (final entity in targets) {
          final updated = switch (entity) {
            TextEntity() when entity.content != text => entity.withContent(
              text,
            ),
  ...
  static CommandDescriptor _justifyText() => CommandDescriptor(
    id: 'edit.justifyText',
    title: 'Justify Text',
    category: _category,
    aliases: const ['justifytext', 'justify'],
    description:
