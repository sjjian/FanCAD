...
- 应用按 `screen` → `services` → (`models` + storage views) → `SettingsStore` → `pkg/*` 分层。依赖只朝下；`pkg` 不得 `import package:fancad/...`。
...
- `screen` 只通过 `Workspace.run` / 已有 service provider 做事，不 import `storage`。
- `services` 不建 Widget。
